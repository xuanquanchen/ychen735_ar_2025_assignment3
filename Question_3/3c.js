    ...
    xr.input.onControllerAddedObservable.add(async (controller) => {
        controller.onMotionControllerInitObservable.add((motionController) => {
            if (motionController.handedness === 'right') {
                ...
                // Right thumbstick for moving the selected model
                    ...
                    rightSqueeze.onButtonStateChangedObservable.add(() => {
                        if (rightSqueeze.pressed) {
                            ...
                            const pickInfo = scene.pickWithRay(ray, (mesh) => {
                                return BallList.includes(mesh);
                            });

                            if (pickInfo.hit && pickInfo.pickedMesh) {
                                if (selectedModel) hl.removeMesh(selectedModel);

                                selectedModel = pickInfo.pickedMesh;
                                hl.addMesh(selectedModel, BABYLON.Color3.White());
                            } else {
                                if (selectedModel) {
                ...
                if (rightThumbstick) {
                    rightThumbstick.onAxisValueChangedObservable.add((axes) => {
                        ...
                        const cameraRotation = xr.baseExperience.camera.rotationQuaternion || BABYLON.Quaternion.Identity();

                        const forward = new BABYLON.Vector3(0, 0, 1);
                        const right = new BABYLON.Vector3(1, 0, 0);

                        const rotatedForward = forward.rotateByQuaternionToRef(cameraRotation, new BABYLON.Vector3());
                        const rotatedRight = right.rotateByQuaternionToRef(cameraRotation, new BABYLON.Vector3());
                        rotatedForward.y = 0;
                        rotatedRight.y = 0;
                        rotatedForward.normalize();
                        rotatedRight.normalize();

                        const movement = rotatedRight.scale(axes.x * movementSpeed).add(rotatedForward.scale(-axes.y * movementSpeed));

                        selectedModel.position.addInPlace(movement);
                    });
                }
            }
        });
